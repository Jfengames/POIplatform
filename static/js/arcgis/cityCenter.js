...
function getCityCenter(cityName,pName){
    var ljCityCenter=new Object();
    var ljaaa=[];
    if (cityName=='北京市'||cityName=='天津'||cityName=='上海'||cityName=='重庆市'){
        for(var i=0;i< cityCenterList.municipalities.length;i++){
            if(cityCenterList.municipalities[i].n==cityName){
                var lja=cityCenterList.municipalities[i].g;
                var ljb=lja.split('|');
                var zoomLayer=ljb[1];
                var ljc=ljb[0];
                var ljd=ljc.split(',');
                ljaaa.push(Number(ljd[0]));
                ljaaa.push(Number(ljd[1]));
               /* ljaaa.push(Number(zoomLayer));*/
                ljCityCenter.center=ljaaa;
                ljCityCenter.zoomLayer=Number(zoomLayer);
                break;
            }
        }
    }else if(cityName=='香港'||cityName=='澳门'||cityName=='台湾'){
        for(var i=0;i< cityCenterList.other.length;i++){
            if(cityCenterList.other[i].n==cityName){
                var lja=cityCenterList.other[i].g;
                var ljb=lja.split('|');
                var zoomLayer=ljb[1];
                var ljc=ljb[0];
                var ljd=ljc.split(',');
        ...
        for(var i=0;i< cityCenterList.provinces.length;i++){
            if(cityCenterList.provinces[i].n==pName){
                for(var j=0;j< cityCenterList.provinces[i].cities.length;j++){
                    if(cityCenterList.provinces[i].cities[j].n==cityName){
                        var lja=cityCenterList.provinces[i].cities[j].g;
                        var ljb=lja.split('|');
                        var zoomLayer=ljb[1];
                        var ljc=ljb[0];
                        var ljd=ljc.split(',');
